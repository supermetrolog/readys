...
class thumbnail {
	...
	function size_height($size=100) {
        ...
        if ($this->img["y"]>0) 
        {
            if ($this->wh_43) $this->img["x_thumb"]=$this->img["y_thumb"]*4/3; else $this->img["x_thumb"] = ($this->img["y_thumb"]/$this->img["y"])*$this->img["x"];
        } 
        else 
        {
	...
	function size_width($size=100)  {
        ...
        if ($this->img["x"]>0) 
        {
            if ($this->wh_43) $this->img["y_thumb"]=$this->img["x_thumb"]/4*3; else $this->img["y_thumb"] = ($this->img["x_thumb"]/$this->img["x"])*$this->img["y"];
        } 
        else 
        {
    ...
    function process () {
        ...
        if ($this->wh_43)
        {
            if ($X_des >= $Y_des*4/3) $X_des=$Y_des*4/3; else $Y_des=$X_des*3/4;
        }
   		...
   		if ($this->checkgd2()) 
        {
            ...
            if (!$this->bicubic_resample) 
            {
                if ($this->wh_43)
                {
                    if ($N_x >= $N_y*4/3) $N_x=$N_y*4/3; else $N_y=$N_x*3/4;
                }
    ...
    **/
    ...
    /**
     *set exact width and height for thumbnail
     *@access public
     *@param integer height
     *@return boolean
     **/
    function size_exact($width=100, $height=80) {
    	if ($this->error_msg) return false;
    
    	//width
    	$this->img["x_thumb"]=$width;
    	//height
    	$this->img["y_thumb"]=$height;
    	if ($this->img["y"]>0) {
    		if ($width/$this->img["x"]>$height/$this->img["y"]) {
    			$new_width = $width;
    			$new_height = $new_width * $this->img["y"] / $this->img["x"];
    		} else {
    			$new_height = $height;
    			$new_width = $new_height * $this->img["x"] / $this->img["y"];
    		}
    		if ($new_width>$width) {
    			$from_x = ($new_width-$width)/2;
    			$from_y = 0;
    		} else {
    			$from_x = 0;
    			$from_y = ($new_height-$height)/2;
    		}
    		$new_src = imagecreatetruecolor($new_width, $new_height);
    		@imagecopyresampled($new_src, $this->img["src"], 0, 0, 0, 0, $new_width, $new_height, $this->img["x"], $this->img["y"]);
    		$this->img["src"] = imagecreatetruecolor($width, $height);
    		imagecopyresampled($this->img["src"], $new_src, 0 , 0, $from_x, $from_y, $width, $height,$width, $height);
    		$this->img["x"] = $width;
    		$this->img["y"] = $height;
    	...
    	if (!$this->allow_enlarge && $this->img["x_thumb"]>$this->img["x"]) $X_des=$this->img["x"];
    	if (!$this->allow_enlarge && $this->img["y_thumb"]>$this->img["y"]) $Y_des=$this->img["y"];
    }
