$(document).ready(function(){
	...
	$("form .search").exists(function(){
		$(document).on("click", ".filter form .search", function(){
			$("#ajax-loader").show();
			if($(window).width() <= 535){
				var data = $("select[name='type-deal']").val()+"?"+$(".mainfilter form :input[name!='type-deal']").serialize();
			}else{
				var data = $("input[name='type-deal']").val()+"?"+$(".mainfilter form :input[name!='type-deal']").serialize();
			}
